...
{
    public class MelonLoggerWarningText : OwOTextBase
    {
        public override void Patch()
        {
            foreach (MethodInfo method in typeof(MelonLogger).GetMethods(BindingFlags.Public | BindingFlags.Static).Where(method=> method.Name == "LogWarning"))
            {
                try
                {
